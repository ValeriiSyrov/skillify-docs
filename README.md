# Skillify Legal & Documentation

This repository contains legal documents and documentation for the **Skillify** mobile application.

## Contents

- **[Privacy Policy](privacy-policy.md)** — Our data collection, usage, and protection practices.

## GitHub Pages Setup

This repo is published via GitHub Pages. To access the privacy policy via web:

1. Go to repository **Settings** → **Pages**
2. Select **Branch:** `main` (or `master`), **Folder:** `/ (root)`
3. Save — GitHub will provide a public URL like `https://yourusername.github.io/skillify-docs/`
4. Access the privacy policy at: `https://yourusername.github.io/skillify-docs/privacy-policy.md`

Or use a direct link to the raw markdown:
```
https://raw.githubusercontent.com/yourusername/skillify-docs/main/privacy-policy.md
```

## Updates

Changes to legal documents should be reflected in the "Last Updated" date on each document.

---

**Skillify App:** Available on iOS App Store
